...
import { LAST_ACTIVE_BRANCH_KEY } from "./logoutState";
...
function clearLastActiveBranchIfMatch(branchId) {
  if (typeof window === "undefined" || !branchId) return;
  try {
    if (localStorage.getItem(LAST_ACTIVE_BRANCH_KEY) === String(branchId)) {
    ...
    if (result.success) {
      ...
      if (id) {
        if (result.errorCode === "forbidden") {
          clearLastActiveBranchIfMatch(id);
        }
