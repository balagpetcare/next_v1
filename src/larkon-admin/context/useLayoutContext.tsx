...
import type { LayoutState, LayoutType, MenuType, OffcanvasControlType, LayoutOffcanvasStatesType, ThemeType } from '@larkon/types/context'

import useLocalStorage from '@larkon/hooks/useLocalStorage'
import useQueryParams from '@larkon/hooks/useQueryParams'

function applyThemeToRoot(theme: ThemeType, topbarTheme: ThemeType, menuTheme: MenuType['theme'], menuSize: MenuType['size']) {
  const root = document.documentElement
  const body = document.body
  if (!body) return
  root.setAttribute('data-bs-theme', theme)
  body.setAttribute('data-bs-theme', theme)
  root.setAttribute('data-topbar-color', topbarTheme)
  root.setAttribute('data-menu-color', menuTheme)
  root.setAttribute('data-menu-size', menuSize)
}

function removeThemeFromRoot() {
  const root = document.documentElement
  const body = document.body
  if (!body) return
  root.removeAttribute('data-bs-theme')
  body.removeAttribute('data-bs-theme')
  root.removeAttribute('data-topbar-color')
  root.removeAttribute('data-menu-color')
  root.removeAttribute('data-menu-size')
}
...
const LayoutProvider = ({ children }: ChildrenType) => {
  ...
  // Apply theme at document root (html + body) before paint so dashboard content area follows theme
  useLayoutEffect(() => {
    applyThemeToRoot(settings.theme, settings.topbarTheme, settings.menu.theme, settings.menu.size)
    return () => {
      removeThemeFromRoot()
    }
