...
import { activityStreamData } from '@larkon/assets/data/topbar'
import type { OffcanvasControlType } from '@larkon/types/context'
import type { ActivityType } from '@larkon/types/data'
import { toSentenceCase } from '@larkon/utils/change-casing'
import { getActivityIcon, getFileExtensionIcon } from '@larkon/utils/get-icons'
import IconifyIcon from './IconifyIcon'
import SimplebarReactClient from './SimplebarReactClient'

const ActivityItem = ({ title, icon, time, content, files, status, type, variant }: ActivityType) => {
  return (
    <div className="position-relative ps-4">
      <div className="mb-4">
        <span
          className={clsx(
            `position-absolute start-0 translate-middle-x d-inline-flex align-items-center justify-content-center rounded-circle text-light fs-20 bg-${variant}`,
            { 'avatar-sm': !icon },
          )}>
          {icon ? (
            <Image src={icon} height={36} width={36} alt="avatar" className="avatar-sm rounded-circle" />
          ) : type ? (
            <IconifyIcon icon={getActivityIcon(type)} />
          ) : (
          ...
          {files && (
            <div className="bg-light bg-opacity-50 rounded-2 p-2">
              <Row>
                {files.map((file, idx) => {
                  return file.preview ? (
                    <Col lg={4} className="px-1" key={idx}>
                      <Image width={107} height={71} src={file.preview} alt="" className="img-fluid rounded" />
                    </Col>
                  ) : (
                    <div key={(file.name ?? '') + idx} className={clsx('border-end border-light', { 'col-lg-6': files.length > 1 })}>
