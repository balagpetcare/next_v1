...
import { getAllProjects } from '@larkon/helpers/data'
                  ...
                  {projectData.map((item, idx) => (
                    <tr key={idx}>
                      ...
                      <td className="avatar-group">
                        {item.teamMembers.map((avatar, idx) => (
                          <Link href="" className="avatar-group-item" key={idx}>
                            <Image src={avatar} alt="avatar-2" className="img-fluid avatar-xs rounded-circle avatar-border" />
                          </Link>
                        ))}
                      </td>
                      <td>{item.deadlineDate.toLocaleString('en-us', { day: '2-digit', month: 'short', year: 'numeric' })}</td>
                      <td>
                        <ProgressBar
                          now={item.progressValue}
