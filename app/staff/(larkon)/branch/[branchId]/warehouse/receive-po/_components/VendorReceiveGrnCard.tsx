...
import { grnSubmitForConfirmation, grnPrintUrl, purchaseOrderPrintUrl } from "@/lib/api";
...
export type VendorReceiveGrnDraft = {
  ...
  createdAt: string;
  totalQty?: number;
  location?: { id?: number; name?: string; branch?: { name?: string; id?: number } } | null;
  vendor?: { name: string } | null;
  purchaseOrder?: { id: number; poNumber: string } | null;
  vendorReceiveSession?: { status: string; submittedAt?: string | null } | null;
  lines: Array<{
    id: number;
    quantity: number;
    ...
    purchaseOrderLine?: { orderedQty: number } | null;
  }>;
};
    ...
    >
      <div className="card-header d-flex align-items-center justify-content-between flex-wrap gap-2">
        <div>
          {branchId ? (
            <Link
              href={`/staff/branch/${branchId}/warehouse/vendor-receipts/${grn.id}`}
        ...
        <div className="d-flex gap-2 flex-wrap">
          <a href={grnPrintUrl(grn.id, "grn")} target="_blank" rel="noopener" className="btn btn-outline-secondary btn-sm">
