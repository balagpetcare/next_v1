...
import {
  ...
  staffClinicInstrumentIssueLogsList,
...
export default function StaffClinicItemsPage() {
  ...
  const loadLedger = useCallback(async () => {
    ...
    try {
      ...
      const data = await staffClinicItemStockLedger(branchId, { limit: 100 });
      setLedgerData(data);
    } catch {
  ...
  const loadConsumption = useCallback(async () => {
    ...
    try {
      ...
      const data = await staffClinicItemStockConsumption(branchId, { limit: 50 });
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const [stockRes, alertsRes, instrumentRes] = await Promise.all([
        staffClinicItemStock(branchId),
        staffClinicLowStockAlerts(branchId),
        staffClinicInstrumentIssueLogsList(branchId),
      ]);
  ...
  const handleReceive = async () => {
    ...
    try {
      setSaving(true);
      setError("");
      await staffClinicItemStockReceive(branchId, {
  ...
  const handleAdjust = async () => {
    ...
    try {
      setSaving(true);
      setError("");
      await staffClinicItemStockAdjust(branchId, {
        itemId: selectedItemId,
        variantId: selectedVariantId,
        deltaQty: delta,
  ...
  const handleIssueInstrument = async () => {
    ...
    try {
      setSaving(true);
      setError("");
      await staffClinicInstrumentIssueLogCreate(branchId, { itemId: selectedItemId, variantId: selectedVariantId, issuedQty: qty });
  ...
  const handleReturnInstrument = async () => {
    ...
    try {
      setSaving(true);
      setError("");
      await staffClinicInstrumentIssueLogReturn(branchId, returnLogId, {
        returnedQty: qty,
  ...
  if (!branchId) {
    return <div className="p-4"><div className="alert alert-warning">Invalid branch.</div></div>;
  }

  return (
    <div className="dashboard-main-body">
      <div className="d-flex justify-content-between align-items-center mb-4">
        <h1 className="h5 mb-0">Clinic items</h1>
        <div className="d-flex gap-2">
          <button type="button" className="btn btn-primary btn-sm radius-8" onClick={openReceive}>
            <i className="ri-add-line me-1" /> Receive
          </button>
          <button type="button" className="btn btn-outline-primary btn-sm radius-8" onClick={openAdjust}>
      ...
      <div className="d-flex gap-2 mb-3">
        ...
        <button type="button" className={`btn radius-8 ${activeTab === "ledger" ? "btn-primary" : "btn-outline-primary"}`} onClick={() => setActiveTab("ledger")}>Ledger</button>
        <button type="button" className={`btn radius-8 ${activeTab === "consumption" ? "btn-primary" : "btn-outline-primary"}`} onClick={() => setActiveTab("consumption")}>Consumption</button>
      </div>

      {activeTab === "ledger" && (
        <div className="card radius-12 mb-4">
          <div className="card-header bg-transparent p-24"><h6 className="mb-0 fw-semibold">Clinical stock ledger</h6></div>
              ...
              <div className="table-responsive">
                <table className="table table-sm table-hover mb-0">
                  <thead className="table-light">
                    <tr><th>Date</th><th>Item</th><th>Variant</th><th>Type</th><th>Delta</th><th>Balance after</th></tr>
                  </thead>
                  <tbody>
      ...
      {activeTab === "consumption" && (
        <div className="card radius-12 mb-4">
          <div className="card-header bg-transparent p-24"><h6 className="mb-0 fw-semibold">Package-linked consumption</h6></div>
          <div className="card-body p-24">
            {consumptionLoading ? (
              <div className="text-center py-4"><div className="spinner-border text-primary" /><p className="text-muted mt-2 mb-0">Loading…</p></div>
              ...
              <div className="table-responsive">
                <table className="table table-sm table-hover mb-0">
                  <thead className="table-light"><tr><th>Mode</th><th>Case</th><th>Items</th></tr></thead>
                  <tbody>
      ...
      {alerts.length > 0 && (
        <div className="card radius-12 border-warning mb-4">
          <div className="card-body p-24">
            <h6 className="fw-semibold text-warning mb-2"><i className="ri-alert-line me-1" /> Low stock alerts</h6>
            <div className="table-responsive">
              <table className="table table-sm table-hover mb-0">
      ...
      <div className="card radius-12 mt-4">
        ...
        <div className="card-body p-24">
          {openInstrumentLogs.length === 0 ? (
            <p className="text-muted mb-0 small">No open instrument issues. Use Issue to record an instrument issued.</p>
          ) : (
            <div className="table-responsive">
              <table className="table table-sm table-hover mb-0">
                <thead className="table-light">
                  <tr><th>Item</th><th>Variant</th><th>Qty</th><th>Issued at</th><th className="text-end">Action</th></tr>
                </thead>
                <tbody>
                  {openInstrumentLogs.map((l) => (
                    <tr key={Number(l.id)}>
                      ...
                      <td className="text-end">
                        <button type="button" className="btn btn-sm btn-outline-primary radius-8" onClick={() => openReturnInstrument(Number(l.id))}>Return</button>
                      </td>
                    </tr>
                  ))}
                </tbody>
          ...
          <div className="modal-dialog modal-dialog-centered">
            <div className="modal-content radius-12">
              ...
              <div className="modal-footer">
                <button type="button" className="btn btn-outline-secondary radius-8" onClick={closeModal}>Cancel</button>
            ...
            <div className="modal-content radius-12">
              ...
              <div className="modal-body">
                ...
                <div className="mb-3">
                  ...
                  <input type="text" className="form-control radius-8" value={sterilizationStatus} onChange={(e) => setSterilizationStatus(e.target.value)} />
                </div>
                <div className="mb-3">
