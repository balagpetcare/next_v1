...
import ServiceAssignmentEnterpriseView from "./_components/ServiceAssignmentEnterpriseView";
...
/** Set `NEXT_PUBLIC_FEATURE_DOCTOR_ASSIGNMENT_V2=false` to restore legacy matrix-only UI. */
const USE_ENTERPRISE_SERVICE_ASSIGNMENT =
  typeof process !== "undefined" && process.env.NEXT_PUBLIC_FEATURE_DOCTOR_ASSIGNMENT_V2 !== "false";
...
export default function StaffClinicDoctorsServiceAssignmentPage() {
  ...
  const load = useCallback(async () => {
    if (USE_ENTERPRISE_SERVICE_ASSIGNMENT || !branchId || !hasAccess) return;
    setLoading(true);
    setError("");
    try {
  ...
  useEffect(() => {
    if (!USE_ENTERPRISE_SERVICE_ASSIGNMENT) load();
  }, [load]);

  // Open drawer for doctor when arriving with ?memberId= (legacy matrix only)
  useEffect(() => {
    if (USE_ENTERPRISE_SERVICE_ASSIGNMENT) return;
    if (
  ...
  return (
    <PageWorkspace>
      ...
      <PageHeader
        ...
        subtitle={
          USE_ENTERPRISE_SERVICE_ASSIGNMENT
            ? "Doctor-centric workspace: categories, bulk actions, templates, and audit"
            : "Map doctors to clinic services and roles (legacy matrix)"
        }
      />

      {USE_ENTERPRISE_SERVICE_ASSIGNMENT ? (
        <ServiceAssignmentEnterpriseView
          branchId={branchId}
          branch={branch}
          myAccess={myAccess}
