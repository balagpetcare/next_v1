...
export default function StaffNewWriteOffPage() {
  ...
  async function fetchLocations() {
    ...
    try {
      const res = await fetch(
        `${API_BASE_URL}/api/v1/inventory/locations?branchId=${branchId}`,
        { credentials: "include" }
      );
  ...
  async function searchVariants(query: string) {
    ...
    try {
      const res = await fetch(
        `${API_BASE_URL}/api/v1/inventory/variants/search?q=${encodeURIComponent(query)}`,
        { credentials: "include" }
      );
      const data = await res.json();
      if (data.success) {
  ...
  async function handleSubmit() {
    if (!selectedLocation || !reason || lines.length === 0) {
    ...
    try {
      ...
      if (data.success) {
        ...
        router.push(`/staff/branch/${branchId}/inventory/write-offs`);
      } else {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-4">
        <Link href={`/staff/branch/${branchId}/inventory/write-offs`}>
        ...
        <div>
          ...
          <p className="text-muted-foreground">
            Request to write off damaged, stolen, or obsolete stock
          </p>
        </div>
      </div>
      ...
      <div className="grid gap-6 lg:grid-cols-2">
                    ...
                    <SelectContent>
                      {availableLots.map((lot) => (
                        <SelectItem key={lot.id} value={lot.id.toString()}>
                          {lot.lotCode} - {lot.onHandQty} available
                        </SelectItem>
                      ))}
                    </SelectContent>
                  </Select>
        ...
        <div className="space-y-6">
          ...
          <div className="flex gap-4">
            <Link href={`/staff/branch/${branchId}/inventory/write-offs`} className="flex-1">
