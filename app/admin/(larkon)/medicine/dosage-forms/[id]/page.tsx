...
export default function AdminMedicineDosageFormEditPage() {
  ...
  const load = useCallback(async () => {
    const res = await adminMedicineWorkspaceApi.dosageFormsGet(id);
  ...
  useEffect(() => {
    (async () => {
      if (!id || Number.isNaN(id)) {
        router.push(`${ADMIN_MEDICINE_BASE}/dosage-forms`);
        return;
      }
      try {
        setLoading(true);
  ...
  if (loading) {
    return (
      <div className="dashboard-main-body py-5 text-center">
        ...
        <p className="text-muted small mt-3 mb-0">Loading dosage form…</p>
      </div>
    );
  }
  ...
  if (!row) {
    return (
      <div className="dashboard-main-body">
        <Link href={`${ADMIN_MEDICINE_BASE}/dosage-forms`} className="text-muted small d-inline-block mb-3">
          ← Dosage Forms
        </Link>
  ...
  const onSave = async () => {
    ...
    try {
      setBusy(true);
      setError("");
      await adminMedicineWorkspaceApi.dosageFormsPatch(id, { displayName: displayName.trim(), aliasesJson });
      await load();
    } catch (e) {
      setError((e as Error)?.message || "Failed");
    } finally {
  ...
  const submitLifecycleConfirm = async () => {
    ...
    try {
      ...
      if (confirmKind === "toggle") {
        await adminMedicineWorkspaceApi.dosageFormsPatch(id, { isActive: !row.isActive });
        setConfirmKind(null);
        await load();
      } else {
        await adminMedicineWorkspaceApi.dosageFormsArchive(id);
        setConfirmKind(null);
        router.push(`${ADMIN_MEDICINE_BASE}/dosage-forms`);
      }
    } catch (e) {
  ...
  return (
    <div className="dashboard-main-body">
      ...
      <h1 className="h4 mb-1 fw-semibold">{displayName || "Dosage form"}</h1>
        ...
        <div className="card-body p-24">
          ...
          <div className="mb-3">
            <MedicineAliasesJsonField
              ref={aliasesRef}
              idPrefix="dosage-form"
              reloadToken={aliasReloadToken}
              initialValue={row.aliasesJson}
              listLabel="Alternate dosage form names"
              listHint="Other labels (e.g. tablet vs tab) that should match this form in imports and search."
            />
          </div>
      ...
      <MedicineConfirmModal
        ...
        title={row.isActive ? "Deactivate dosage form?" : "Activate dosage form?"}
      ...
      >
        <p className="mb-0">
          {row.isActive
            ? "New master-data composition may reject inactive forms where the API enforces active dosage forms."
            : "This dosage form becomes available for new presentations."}
        </p>
      </MedicineConfirmModal>

      <MedicineConfirmModal
        open={confirmKind === "archive"}
        title="Archive dosage form?"
      ...
      >
        <p className="mb-0">
          Soft archive is audit-logged. Blocked while presentations still reference this form. Not a hard delete.
        </p>
      </MedicineConfirmModal>
    </div>
  );
