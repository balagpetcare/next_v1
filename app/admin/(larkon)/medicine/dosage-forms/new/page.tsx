...
export default function AdminMedicineDosageFormNewPage() {
  ...
  const onSubmit = async (e: React.FormEvent) => {
    ...
    try {
      setBusy(true);
      setError("");
      const res = await adminMedicineWorkspaceApi.dosageFormsCreate({ displayName: displayName.trim() });
      const id = (res.data as { id?: number })?.id;
      if (id) router.push(`${ADMIN_MEDICINE_BASE}/dosage-forms/${id}`);
  ...
  return (
    <div className="dashboard-main-body">
      ...
      <h1 className="h4 mb-3">New dosage form</h1>
