...
export default function AdminMedicineGenericNewPage() {
  ...
  const onSubmit = async (e: React.FormEvent) => {
    ...
    try {
      setBusy(true);
      setError("");
      const res = await adminMedicineWorkspaceApi.genericsCreate({ displayName: displayName.trim() });
      const id = (res.data as { id?: number })?.id;
      if (id) router.push(`${ADMIN_MEDICINE_BASE}/generics/${id}`);
  ...
  return (
    <div className="dashboard-main-body">
      ...
      <h1 className="h4 mb-3">New generic</h1>
