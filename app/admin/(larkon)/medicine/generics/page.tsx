...
export default function AdminMedicineGenericsPage() {
  ...
  const load = useCallback(async () => {
    try {
      setLoading(true);
      const res = await adminMedicineWorkspaceApi.genericsList({
  ...
  return (
    <div className="dashboard-main-body">
      <div className="d-flex flex-wrap justify-content-between gap-3 mb-4">
        <div>
          <h1 className="h4 mb-1">Generics</h1>
          <p className="text-muted small mb-0">Global ingredient / generic names. Archive blocked while presentations exist.</p>
        </div>
        <Link href={`${ADMIN_MEDICINE_BASE}/generics/new`} className="btn btn-primary btn-sm radius-12">
          New generic
        </Link>
      </div>
      {error && <div className="alert alert-danger radius-12 mb-3">{error}</div>}
      <div className="card radius-12 mb-3">
        <div className="card-body p-16 d-flex flex-wrap gap-2 align-items-center">
          <input
                  ...
                  {!loading && items.length === 0 ? (
                    <tr>
                      <td colSpan={5} className="p-0">
                        <MedicineTableEmptyState title="No generics found" hint="Adjust search or include inactive rows." />
                      </td>
                    </tr>
                  ) : (
                    items.map((g, idx) => (
                      <tr key={String(g.id)}>
                        ...
                        <td className="text-end">
                          <Link href={`${ADMIN_MEDICINE_BASE}/generics/${g.id}`} className="btn btn-sm btn-outline-primary radius-8">
                            Open
                          </Link>
                        </td>
                      </tr>
