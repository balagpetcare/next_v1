...
export default function AdminMedicineGenericEditPage() {
  ...
  const load = useCallback(async () => {
    const res = await adminMedicineWorkspaceApi.genericsGet(id);
  ...
  useEffect(() => {
    (async () => {
      if (!id || Number.isNaN(id)) {
        router.push(`${ADMIN_MEDICINE_BASE}/generics`);
        return;
      }
      try {
        setLoading(true);
  ...
  if (loading) {
    return (
      <div className="dashboard-main-body py-5 text-center">
        ...
        <p className="text-muted small mt-3 mb-0">Loading generic…</p>
      </div>
    );
  }
  ...
  if (!row) {
    return (
      <div className="dashboard-main-body">
        <Link href={`${ADMIN_MEDICINE_BASE}/generics`} className="text-muted small d-inline-block mb-3">
          ← Generics
        </Link>
  ...
  const onSave = async () => {
    ...
    try {
      setBusy(true);
      setError("");
      await adminMedicineWorkspaceApi.genericsPatch(id, { displayName: displayName.trim(), aliasesJson });
      await load();
    } catch (e) {
      setError((e as Error)?.message || "Failed");
    } finally {
  ...
  const submitLifecycleConfirm = async () => {
    ...
    try {
      ...
      if (confirmKind === "toggle") {
        await adminMedicineWorkspaceApi.genericsPatch(id, { isActive: !row.isActive });
        setConfirmKind(null);
        await load();
      } else {
        await adminMedicineWorkspaceApi.genericsArchive(id);
        setConfirmKind(null);
        router.push(`${ADMIN_MEDICINE_BASE}/generics`);
      }
    } catch (e) {
  ...
  return (
    <div className="dashboard-main-body">
      ...
      <h1 className="h4 mb-1 fw-semibold">{displayName || "Generic"}</h1>
        ...
        <div className="card-body p-24">
          ...
          <div className="mb-3">
            <MedicineAliasesJsonField
              ...
              listHint="Other spellings or synonyms that should match this generic in imports and search. One name per line."
            />
          </div>
      ...
      <MedicineConfirmModal
        ...
        title={row.isActive ? "Deactivate generic?" : "Activate generic?"}
      ...
      >
        <p className="mb-0">
          {row.isActive
            ? "This generic will be hidden from new composition flows where the API enforces active masters. Existing presentations are unchanged."
            : "This generic becomes available again for new presentations and listings where masters must be active."}
        </p>
      </MedicineConfirmModal>

      <MedicineConfirmModal
        open={confirmKind === "archive"}
        title="Archive generic?"
      ...
      >
        <p className="mb-0">
          Soft archive is audit-logged. The API blocks this if any presentations still reference this generic. This is not a hard delete.
        </p>
      </MedicineConfirmModal>
    </div>
  );
