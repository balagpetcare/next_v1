...
import { getEmailDetails } from '@larkon/helpers/data'
...
const FilePreview = ({ file }: { file: FileType }) => {
  return (
    <Card className="p-2 mb-0 shadow-none border position-relative h-100">
      {file.preview ? (
        <Image src={file.preview} height={90} width={140} className="rounded bg-light" alt={file.name ?? ''} />
      ) : (
        <div className="rounded bg-light text-center flex-centered fs-1" style={{ height: 90, width: 140 }}>
...
<p>End of air-mode area</p>
  ...
  return (
    <Offcanvas show={open} onHide={toggle} placement="end" className="email-offcanvas w-50">
      <OffcanvasHeader>
