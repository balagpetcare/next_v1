...
import { getAllTasks } from '@larkon/helpers/data'
        ...
        <CardBody className="p-0 pb-3">
          <SimplebarReactClient className="p-3" style={{ maxHeight: 386 }}>
            {tasksData.map((item, idx) => (
              <div className={`form-check form-todo ${idx == 0 ? 'ps-4' : 'py-1 my-2 ps-4'}  `} key={idx}>
                {item.status == 'Completed' ? (
                  <input type="checkbox" className="form-check-input rounded-circle mt-0 fs-18" defaultChecked id="customCheck2" />
                ) : (
                  <input type="checkbox" className="form-check-input rounded-circle mt-0 fs-18" id="customCheck1" />
                )}
