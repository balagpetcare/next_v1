...
import { getAllReviews } from '@larkon/helpers/data'
import { ReviewListType } from '@larkon/types/data'
import Image from 'next/image'
import doubleImg from '@larkon/assets/images/double.png'
import { getRatingStatus } from '@larkon/utils/other'
...
const ReviewCard = ({ description, rating, title, user }: ReviewListType) => {
  return (
    <Card className="overflow-hidden">
      ...
      <CardFooter className="bg-primary position-relative mt-3">
        <div className="position-absolute top-0 start-0 translate-middle-y ms-3">
          {user?.image && (
            <Image src={user?.image} alt="avatar" width={72} height={72} className="avatar-lg border border-light border-3 rounded-circle" />
          )}
        </div>
