import userProfile from '@larkon/assets/images/user-profile.png'
import avatar2 from '@larkon/assets/images/users/avatar-2.jpg'
...
import { latestInvoice, LatestInvoiceType } from './data'
...
const LatestInvoiceCard = ({ title, date }: LatestInvoiceType) => {
  return (
    <div className="d-flex p-2 rounded align-items-center gap-2 bg-light-subtle">
      ...
      <div className="d-block">
        ...
        <p className="text-muted mb-0 fs-13">{date.toLocaleString('en-us', { day: 'numeric', month: 'short', year: 'numeric' })}</p>
      </div>
      ...
      <Row>
        <Col lg={4}>
          ...
          <Card>
            ...
            <CardBody>
              {latestInvoice.map((item, idx) => (
                <LatestInvoiceCard key={idx} {...item} />
              ))}
            </CardBody>
          </Card>
        </Col>
            ...
            <CardBody>
              <div className="table-responsive">
                <table className="table align-middle mb-0 table-hover table-centered">
                  ...
                  <tbody>
                    {transactionHistoryData.slice(0, 5).map((item, idx) => (
                      <tr key={idx}>
                        <td>
                        ...
                        <td>
                          {' '}
                          <span
                            className={`badge bg-${item.orderStatus == 'Canceled' ? 'danger' : item.orderStatus == 'Packaging' ? 'primary' : item.orderStatus == 'Draft' ? 'warning' : 'success'}-subtle text-${item.orderStatus == 'Canceled' ? 'danger' : item.orderStatus == 'Packaging' ? 'primary' : item.orderStatus == 'Draft' ? 'warning' : 'success'} py-1 px-2`}>
                            {item.orderStatus}
                          </span>{' '}
                        </td>
                        <td> ${item.total}.00 </td>
