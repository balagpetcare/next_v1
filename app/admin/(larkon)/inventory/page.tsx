...
export default function AdminInventoryPage() {
  ...
  const [expiring, setExpiring] = useState<Record<number, any[]>>({ 7: [], 15: [], 30: [] })
  const [lowStockOnly, setLowStockOnly] = useState(false)
  const [search, setSearch] = useState('')
  const [expiryBucket, setExpiryBucket] = useState<number>(7)
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setInventory(Array.isArray((inv as any)?.data) ? (inv as any).data : (inv as any)?.data?.items ?? [])
      setSummary((stock as any)?.data ?? null)
      setExpiring({
        7: Array.isArray((exp7 as any)?.data) ? (exp7 as any).data : [],
        15: Array.isArray((exp15 as any)?.data) ? (exp15 as any).data : [],
        30: Array.isArray((exp30 as any)?.data) ? (exp30 as any).data : [],
      })
    } catch (e) {
  ...
  const filtered = useMemo(() => {
    ...
    return inventory.filter((item) => {
      ...
      const branchName = String(item?.branch?.name ?? '').toLowerCase()
  ...
  return (
    <AdminPageShell
      ...
      actions={
        <button type="button" className="btn btn-outline-primary btn-sm d-flex align-items-center gap-2" onClick={load} disabled={loading}>
      ...
      {summary ? (
        <div className="row g-3 mb-3">
          ...
          <div className="col-12 col-sm-6 col-lg-3">
            <StatCard
              title="Total Value"
              value={summary?.summary?.totalValue != null ? `৳${Number(summary.summary.totalValue).toLocaleString()}` : '—'}
        ...
        <div className="d-flex flex-wrap gap-2 mb-3">
          {EXPIRY_BUCKETS.map((d) => (
            <button
              key={d}
              type="button"
              className={`btn btn-sm ${expiryBucket === d ? 'btn-primary' : 'btn-outline-secondary'}`}
              onClick={() => setExpiryBucket(d)}
            >
              {d} days
            </button>
          ))}
              ...
              {expiringList.map((item) => (
                ...
                >
                  <td>
                    ...
                    <div className="text-secondary" style={{ fontSize: 12 }}>{item?.variant?.title ?? item?.variant?.sku ?? '—'}</div>
                  </td>
                  <td style={{ fontSize: 13 }}>{item?.branch?.name ?? `Branch #${item?.branchId}`}</td>
                  <td>{item?.quantity ?? 0}</td>
                  <td style={{ fontSize: 13 }}>{item?.expiryDate ? new Date(String(item.expiryDate)).toLocaleDateString() : '—'}</td>
                </tr>
              ))}
              {!expiringList.length && !loading ? (
                <tr>
                  <td colSpan={4} className="text-secondary text-center">None expiring in ≤{expiryBucket} days.</td>
                </tr>
              ) : null}
            </tbody>
          </table>
          ...
          <FilterPanel title="Filters">
            <div className="d-flex flex-column gap-2">
              ...
              <input
                ...
                placeholder="Product, SKU, branch..."
                value={search}
                  ...
                  {filtered.map((item) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <td style={{ fontSize: 13 }}>{minStock}</td>
                        <td>
                          {isOutOfStock ? (
                            <StatusChip status="OUT_OF_STOCK" />
                          ) : isLowStock ? (
                            <StatusChip status="LOW_STOCK" />
                          ) : (
                            <StatusChip status="IN_STOCK" />
                          )}
                        </td>
                      </tr>
                    )
