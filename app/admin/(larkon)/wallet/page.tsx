...
interface Withdrawal {
  ...
  status: string
  createdAt?: string
  payoutDetails?: { bankName?: string; accountType?: string }
}
...
export default function WalletPage() {
  ...
  async function load() {
    ...
    try {
      ...
      const [w, s] = await Promise.all([
        apiGet<{ data?: Withdrawal[] }>(`/api/v1/wallet/admin/withdraw/requests?${params.toString()}`).catch(() => ({ data: [] })),
        apiGet<{ data?: unknown }>('/api/v1/admin/dashboard/summary').catch(() => ({ data: null })),
      ])

      setWithdrawals(Array.isArray(w?.data) ? w.data : [])
      setSummary((s as { data?: unknown })?.data ?? null)
    } catch (e) {
      setError((e as Error)?.message || 'Failed to load withdrawal requests')
    } finally {
      setLoading(false)
    }
  }
  ...
  const filtered = useMemo(() => withdrawals, [withdrawals])

  const handleStatusUpdate = async (requestId: number, newStatus: string) => {
    if (!confirm(`Update withdrawal request status to ${newStatus}?`)) return
    try {
      await apiPatch(`/api/v1/wallet/admin/withdraw/requests/${requestId}/status`, { status: newStatus })
      await load()
    } catch (e) {
            ...
            <tbody>
              {filtered.map((r) => (
                <tr key={r.id}>
                  ...
                  <td>
                    <div style={{ fontSize: 13 }}>User #{r.userId}</div>
                  </td>
                  <td>
                  ...
                  <td className="text-end">
                    <div className="d-flex gap-2 justify-content-end">
                      <Link className="btn btn-sm btn-primary" href={`/admin/wallet/${r.id}`}>
                        View
                      </Link>
                      {r.status === 'SUBMITTED' && (
                        <>
