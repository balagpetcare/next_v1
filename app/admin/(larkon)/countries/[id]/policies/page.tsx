...
export default function CountryPoliciesPage() {
  ...
  const onCreate = async (e: React.FormEvent) => {
    ...
    setError('')
    try {
      await apiPost(`/api/v1/admin/country/countries/${countryId}/policies`, {
  ...
  const activate = async (policyId: number) => {
    ...
    setError('')
    try {
      await apiPost(`/api/v1/admin/country/policies/${policyId}/activate`, {})
      await load()
    } catch (e) {
  ...
  const archive = async (policyId: number) => {
    ...
    setError('')
    try {
      await apiPatch(`/api/v1/admin/country/policies/${policyId}`, { status: 'ARCHIVED' })
      await load()
    } catch (e) {
  ...
  return (
    <AdminPageShell
      ...
      breadcrumbs={[{ label: 'Geography' }, { label: 'Countries', href: '/admin/countries' }, { label: `#${countryId}` }, { label: 'Policies' }]}
    >
        ...
        <form className="row g-3" onSubmit={onCreate}>
          <div className="col-md-6">
            ...
            <input
              ...
              placeholder="Bangladesh Default Policy"
            />
          </div>
              ...
              {rows.map((row) => (
                <tr key={row.id}>
                  ...
                  <td className="d-flex gap-2 flex-wrap">
                    <Link className="btn btn-sm btn-outline-primary" href={`/admin/countries/${countryId}/policies/${row.id}`}>View</Link>
