                ...
                <tbody>
                  {couponsData.map((item, idx) => (
                    <tr key={idx}>
                      ...
                      <td>{item.endDate.toLocaleString('en-us', { day: 'numeric', month: 'short', year: 'numeric' })}</td>
                      <td>
                        <span
                          className={`badge text-${item.productStatus == 'Expired' ? 'danger' : 'success'} bg-${item.productStatus == 'Expired' ? 'danger' : 'success'}-subtle fs-12`}>
