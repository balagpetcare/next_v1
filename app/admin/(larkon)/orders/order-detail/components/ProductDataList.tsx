            ...
            <tbody>
              {productData.slice(0, 4).map((item, idx) => (
                <tr key={idx}>
                  ...
                  <td>
                    <span
                      className={`badge bg-${item.orderStatus == 'Packaging' ? 'light' : item.orderStatus == 'Canceled' ? 'danger-subtle' : 'success-subtle'} text-${item.orderStatus == 'Packaging' ? 'dark' : item.orderStatus == 'Canceled' ? 'danger' : 'success'}  px-2 py-1 fs-13`}>
                      {item.orderStatus}
                    </span>
                  </td>
