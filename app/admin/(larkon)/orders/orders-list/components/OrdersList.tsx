              ...
              <table className="table align-middle mb-0 table-hover table-centered">
                ...
                <tbody>
                  {ordersData.map((item, idx) => (
                    <tr key={idx}>
                      <td>#{item.id}/80</td>
                      <td>{item.product?.date.toLocaleString('en-us', { day: 'numeric', month: '2-digit', year: 'numeric' })}</td>
                      <td>
                      ...
                      <td>
                        {' '}
                        <span
                          className={`badge border border-${item.orderStatus == 'Draft' ? 'secondary' : item.orderStatus == 'Canceled' ? 'danger' : item.orderStatus == 'Packaging' ? 'warning' : 'success'} text-${item.orderStatus == 'Completed' ? 'success' : item.orderStatus == 'Canceled' ? 'danger' : item.orderStatus == 'Packaging' ? 'warning' : 'secondary'}  px-2 py-1 fs-13`}>
                          {item.orderStatus}
                        </span>
                      </td>
                      <td>
