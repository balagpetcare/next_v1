...
import {
  ownerClinicSterilizationCyclesList,
  ownerClinicSterilizationCycleStart,
  ownerClinicSterilizationCycleComplete,
  ownerClinicSterilizationCycleFail,
  ownerClinicInstrumentInstancesList,
  ownerClinicSterilizationDueAlerts,
...
export default function OwnerClinicSterilizationPage() {
  ...
  const loadCycles = useCallback(async () => {
    if (!branchId) return;
    try {
      const data = await ownerClinicSterilizationCyclesList(branchId, {
  ...
  const loadInstances = useCallback(async () => {
    if (!branchId) return;
    try {
      const data = await ownerClinicInstrumentInstancesList(branchId);
  ...
  const loadDue = useCallback(async () => {
    if (!branchId) return;
    try {
      const data = await ownerClinicSterilizationDueAlerts(branchId);
  ...
  const handleStartCycle = async () => {
    ...
    setError("");
    try {
      await ownerClinicSterilizationCycleStart(branchId, {
  ...
  const handleComplete = async (cycleId: number) => {
    ...
    setError("");
    try {
      await ownerClinicSterilizationCycleComplete(branchId, cycleId, { sterileDays: 7 });
      loadCycles();
      loadDue();
    } catch (e) {
  ...
  const handleFail = async (cycleId: number) => {
    ...
    setError("");
    try {
      await ownerClinicSterilizationCycleFail(branchId, cycleId);
      loadCycles();
      loadDue();
    } catch (e) {
  ...
  return (
    <div className="dashboard-main-body">
      <PageHeader
        title="Sterilization"
        subtitle="Instrument sterilization cycles and status"
        breadcrumbs={[
          { label: "Home", href: "/owner" },
          { label: "Clinic", href: "/owner/clinic" },
          { label: "Branch", href: "/owner/clinic/" + branchId },
          { label: "Sterilization", href: "/owner/clinic/" + branchId + "/sterilization" },
        ]}
      />
